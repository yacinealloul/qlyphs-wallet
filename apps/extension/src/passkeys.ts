/** WebAuthn is invoked only in a dedicated extension tab so the toolbar popup may close. */
...
export async function passkeyPRF(
  ...
  if (!['chrome-extension:', 'moz-extension:'].includes(location.protocol))
    throw Error(
      'Passkeys are available in the installed extension. This preview does not access your authenticators.',
    );
  ...
  if (enroll) {
    const created = (await navigator.credentials.create({
      ...
      publicKey: {
        ...
        rp: { name: 'Qlyphs Wallet (Development)' },
        user: {
          id: buffer(challenge.owner),
          name: 'Qlyphs development wallet',
...
export function passkeyError(error: unknown): Error {
  if (error instanceof DOMException) {
    ...
    if (['NotSupportedError', 'SecurityError'].includes(error.name))
      return Error(
        'Passkeys are not supported in this extension on your device. Use your password.',
      );
